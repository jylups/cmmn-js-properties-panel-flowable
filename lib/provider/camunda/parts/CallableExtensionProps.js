...
var attributeInfo = {
  ...
  'cmmn:ProcessTask': {
    binding: 'flowable:processBinding',
    version: 'flowable:processVersion',
    tenantId: 'flowable:processTenantId'
  },

  'cmmn:DecisionTask': {
    binding: 'flowable:decisionBinding',
    version: 'flowable:decisionVersion',
    tenantId: 'flowable:decisionTenantId'
  }
};
...
module.exports = function(group, element, cmmnFactory, elementRegistry, itemRegistry, translate) {
  ...
  group.entries.push(entryFactory.textField({
    ...
    get: getValue('resultVariable', 'flowable:resultVariable'),
    ...
    set: function(element, values, node, bo) {
      ...
      if (is(bo, 'flowable:DecisionTask') && !resultVariable) {
        assign(props, {
    ...
    disabled: function(element, node) {
      var definition = getDefinition(element);
      return !is(definition, 'flowable:DecisionTask');
    }
  ...
  group.entries.push(entryFactory.selectBox({
    ...
    get: getValue('mapDecisionResult', 'flowable:mapDecisionResult', 'resultList'),

    set: function(element, values, node, bo) {
      var update = { 'flowable:mapDecisionResult': values.mapDecisionResult || 'resultList' };
      return cmdHelper.updateProperties(bo, update, element);
    },

    disabled: function(element, node) {
      var definition = getDefinition(element);
      var resultVariable = definition && definition.get('flowable:resultVariable');
      return !(is(definition, 'flowable:DecisionTask') && typeof resultVariable !== 'undefined');
    }
