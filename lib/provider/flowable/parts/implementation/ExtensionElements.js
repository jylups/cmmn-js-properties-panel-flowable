...
function getSelectBox(node, id) {
  ...
  var query = 'select[name=selectedExtensionElement]' + (id ? '[id=flowable-extensionElements-' + id + ']' : '');
...
module.exports = function(element, cmmnFactory, options) {
  ...
  return {
    ...
    html: '<div class="cpp-row cpp-element-list" ' +
            ...
            '<label for="flowable-extensionElements-' + escapeHTML(id) + '">' + escapeHTML(label) + '</label>' +
            '<div class="cpp-field-wrapper">' +
              '<select id="flowable-extensionElements-' + escapeHTML(id) + '"' +
              ...
              (canCreate ? '<button class="add" ' +
                                   'id="flowable-extensionElements-create-' + escapeHTML(id) + '" ' +
                                   'data-action="createElement">' +
                             '<span>+</span>' +
                           '</button>' : '') +
              (canRemove ? '<button class="clear" ' +
                                   'id="flowable-extensionElements-remove-' + escapeHTML(id) + '" ' +
